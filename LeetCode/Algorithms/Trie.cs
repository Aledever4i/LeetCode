...
namespace LeetCode.Algorithms.Trie
{
    public sealed class TrieDictionary<TValue>(IEqualityComparer<char> comparer = null) : IDictionary<string, TValue>, IReadOnlyDictionary<string, TValue>
    {
        private readonly Trie _trie = new(comparer);

        public ICollection<string> Keys => [.. _trie];

        public ICollection<TValue> Values => [.. _trie.GetAllTerminalNodes().Select(t => ((TerminalValueCharTrieNode)t).Value)];
    ...
    {
        public static void ThrowIfNullOrEmpty(ReadOnlySpan<char> argument, string paramName = null)
        {
            if (argument.IsEmpty)
            {
